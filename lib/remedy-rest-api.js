/*
    remedy-rest-api.js (v 2.51)
    Amy Hicox <amy@hicox.com> 3/17/22
*/
...
/*
    added in v2.52 (4/19/21)

    createWebHook({
        schema:              <str, required>,
        operations:          <array(enum - one or all of): create | update | delete>,
        QBE:                 <str(QBE), optional>,
        fields:              <array, optional>,
        endpoint:            <str(URL), required>,
...
createWebHook(p){
    let that = this;
    let functionName = 'createWebHook';
    ...
    return(new Promise(function(toot, boot){
        ...
        if (! that.isAuthenticated){
            ...
            // bounce for missing args
            let missingArgs = [];
            ['protocol', 'server', 'port', 'schema', 'operations', 'endpoint', 'description'].forEach(function(arg){
                if (! (args.hasOwnProperty(arg) && that.isNotNull(args[arg]))){ missingArgs.push(arg); }
            });
            if (missingArgs.length > 0){
                boot(new RemedyRestAPIException({
                ...
                // bounce for operaions not in range or not specified
                let opsError = false;
                let opsErrorStr = null;
                if (
                    (args.hasOwnProperty('operations')) &&
                    (args.operations instanceof Array) &&
                    (args.operations.length > 0)
                ){
                    let tmp = [];
                    args.operations.forEach(function(op){
                        if (! (['create', 'update', 'delete'].indexOf(op) >= 0)){ tmp.push(op); }
                    });
                    if (tmp.length > 0){
                        opsError    = true;
                        opsErrorStr = `specified unknown value(s) on 'operations' argument: ${tmp.join(", ")}`;
                    }
                }else{
                    opsError    = true;
                    opsErrorStr = `'operations' argument not specified or not array`
                }
                if (opsError){
                    boot(new RemedyRestAPIException({
                        messageType: 'non-ars',
                        message: `${that._className} v${that._version} | ${functionName}() | ${opsErrorStr}`,
                        thrownByFunction: `${that._className} v${that._version} | ${functionName}()`,
                        thrownByFunctionArgs: args
                    }));
                }else{
                    ...
                    let postBody = {
                        ...
                        form_name:      args.schema,
                        entry_events:   args.operations,
                        callback: {
                            url:    args.endpoint
                        }
                    }
                    if (that.isNotNull(args.QBE)){ postBody.condition = args.QBE; }
                    if (args.hasOwnProperty('fields') && (args.fields instanceof Array) && (args.fields.length > 0)){
                        postBody.payload_fields = args.fields;
                    }
                    if (
                        args.hasOwnProperty('basic_auth_user') &&
                        that.isNotNull(args.basic_auth_user) &&
                        args.hasOwnProperty('basic_auth_password') &&
                        that.isNotNull(args.basic_auth_password)
                    ){
                        postBody.callback.authentication = {
                            type:       "basic",
                    ...
                    /*
                        4/19/22 - yargh! this does not work! Opened issue with BMC on this.
                        for now, you'll have to just call a modifyTicket on the AR System Webhook
                        entryId this spits back and set the 'Headers' field to whatever you need
                        in the header
                     */
                    // if (that.isNotNull(args.headers)){ postBody.headers = args.headers; }

                    //  "don't 'dude' me, bro!" - Ghost Adventures
                    let abort = false;
                    let url = `${args.protocol}://${args.server}:${args.port}${(that.hasAttribute('proxyPath'))?that.proxyPath:''}/api/arsys/v1.0/webhook`;
                    if (that.debug){ console.log(`${that._className} v${that._version} | ${functionName}() | ${url}`); }
                    that.apiFetch({
                        endpoint:           url,
                        method:             'POST',
...
getWebHook(p){
    ...
    return(new Promise(function(toot, boot){
        ...
        if (! that.isAuthenticated){
            ...
            // bounce for missing args
            let missingArgs = [];
            ['protocol', 'server', 'port', 'webHookID'].forEach(function(arg){
                if (! (args.hasOwnProperty(arg) && that.isNotNull(args[arg]))){ missingArgs.push(arg); }
            });
            if (missingArgs.length > 0){
                boot(new RemedyRestAPIException({
                ...
                let abort = false;
                let url = `${args.protocol}://${args.server}:${args.port}${(that.hasAttribute('proxyPath'))?that.proxyPath:''}/api/arsys/v1.0/webhook/${args.webHookID}`;
                if (that.debug){ console.log(`${that._className} v${that._version} | ${functionName}() | ${url}`); }
                that.apiFetch({
                    endpoint:           url,
                    method:             'GET',
...
/*
    ...
    modifyWebHook({
        webHookID:           <str(entryId), required>,
        enabled:             <bool>,
        schema:              <str, required>,
        operations:          <array(enum - one or all of): create | update | delete>,
        QBE:                 <str(QBE), optional>,
...
modifyWebHook(p){
    let that = this;
    let functionName = 'modifyWebHook';
        ...
        if (! that.isAuthenticated){
            ...
            if (missingArgs.length > 0){
                ...
                if (opsError){
                    ...
                    // get the post body together
                    let postBody = { webhook_id: args.webHookID }
                    if (args.hasOwnProperty('description') && that.isNotNull(args.description)){ postBody.description = args.description; }
                    if (args.hasOwnProperty('schema') && that.isNotNull(args.schema)){ postBody.form_name = args.schema; }
                    if ((args.hasOwnProperty('operations')) && (args.operations instanceof Array) && (args.operations.length > 0)){ postBody.entry_events = args.operations; }
                    if (args.hasOwnProperty('endpoint') && that.isNotNull(args.endpoint)){

                        // note: if you wanna change user/pass you've gotta specify the endpoint again too for the datastructure to work

                        postBody.callback = { url: args.endpoint };
                        if (
                            args.hasOwnProperty('basic_auth_user') &&
                            that.isNotNull(args.basic_auth_user) &&
                            args.hasOwnProperty('basic_auth_password') &&
                    ...
                    /*
                        4/19/22 - yeah doesn't work on modify either, waitin' to hear from BMC
                     */
                    // if (that.isNotNull(args.headers)){ postBody.headers = args.headers; }
...
deleteWebHook(p){
    let that = this;
    let functionName = 'deleteWebHook';
