...
The previous version of this library was based on the [XHR API](https://developer.mozilla.org/en-US/docs/Web/API/XMLHttpRequest), which required an external node module to emulate the `XMLHttpRequest` object. Though mature, the XHR API was quite complicated to use. By moving to Fetch API, it was possible to remove the external dependency in the Node environment, though in doing so deprecated the `progressCallback` option on `getTicket()`. So if you need to draw a progress indicator on particularly large fetches from the api, you might need to use the older version, which is available in this package under the `./legacy` directory.
